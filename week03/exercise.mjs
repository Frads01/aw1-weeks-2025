...
function Question(id, text, email, userId, date) {
  ...
  this.getAnswers = () => {
    return new Promise((resolve, reject) => {
      ...
      const sql = "SELECT answer.*, user.email FROM answer JOIN user ON answer.authorId = user.authorId WHERE answer.questionId = ?";
      db.all(sql, [id], (err, rows) => {
        if (err) 
          reject(err);

        resolve(rows.map(row => new Answer(
          row.id,
          row.text,
          row.email,
          row.authorId,
...
function QuestionList() {
  ...
  this.getQuestion = (id) => {
    return new Promise((resolve, reject) => {
      ...
      const sql = "SELECT question.*, user.email FROM question JOIN user ON question.authorId = user.authorId WHERE question.id = ?";
